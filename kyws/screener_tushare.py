    ...
    while (try_times < 5) and (len(my_list) == 0):
        try:
            ...
            for each in results:
                ...
                stock_code = stock_symbol[-8:-2] + '.' + stock_symbol[-10:-8]
                # print(stock_code)
...
def sendMail(mail_content,subject="Today's A share chances"):
    ...
    mail_pw = 'xxxxxxxxxxxxxxxxxx'
    mail_recv = ['xxxxxxxxxxxxx@xxx.com']
...
class Alchemist(object):
    ...
    def getData(self):
        try:
            self.hist_data = ts.pro_bar(ts_code=self.code, start_date=self.start_date, end_date=self.end_date,adj='hfq')
            if self.hist_data.shape[0] < 2*250:    # 忽略上市时间太短的个股
                return 'too young'
            else:
                self.hist_data = self.hist_data.sort_values('trade_date')
                self.fast_line = self.hist_data['close'].rolling(self.fast_line_para).mean()
                self.month_line = self.hist_data['close'].rolling(self.month_line_para).mean()
                self.main_line = self.hist_data['close'].rolling(self.main_line_para).mean()
                self.year_line = self.hist_data['close'].rolling(self.year_line_para).mean()
                return 'success'
        except Exception as e:
            # print(code)
            # print(e)
    ...
    def checkBuy(self):
        ...
        if data_res == 'success':
            ...
            cond_1 = 1.08*self.year_line.values[-1] > self.hist_data['close'].values[-1] > self.year_line.values[-1]    # 收盘价在年线上方不远
            ...
            cond_2 = max(self.hist_data['low'].values[-10:] - self.hist_data['high'].values[-11:-1]) <= 0    # 最近十日无向上跳空
    ...
    def checkSell(self):
        ...
        if data_res == 'success':
            ...
            rise_range_cond = self.hist_data['close'].values[-1] > 1.2*self.year_line.values[-1]
...
pro = ts.pro_api('xxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
